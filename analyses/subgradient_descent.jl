...
function adam(
    ...
    vt = zeros(length(diffmodel.θ))
    mt = zeros(length(diffmodel.θ))
    vhat = zeros(length(diffmodel.θ))
    mhat = zeros(length(diffmodel.θ))
    dLdθ = zeros(length(diffmodel.θ))
    dθ = zeros(length(diffmodel.θ))
    dLdx = zeros(size(diffmodel.Q(diffmodel.θ), 1))

    loss_offset = 0.5 * length(diffmodel.var_ids)
    ...
    for iter = 1:niters
        ...
        mt .= β1 * mt .+ (1.0 - β1) * dLdθ
        vt .= β2 * vt .+ (1.0 - β2) * dLdθ .^ 2
        mhat .= mt ./ (1.0 - β1^iter)
        vhat .= vt ./ (1.0 - β2^iter)
        dθ .= mhat ./ (sqrt.(vhat) .+ ϵ)
        diffmodel.θ .-= map(cap, diffmodel.θ, η .* dθ)
        ...
        (iter % save_on_iter == 0 || iter in save_iters_extra) &&
            save_data(savedir, iter, diffmodel.θ, dLdθ, loss)
